...
from datasets import IDX_TO_SUBMISSION, TestImageDataset, CLASSES
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run inference on test set and build CSV submission.")
    parser.add_argument("--data-root", type=Path, default=DEFAULT_DATA_ROOT)
    parser.add_argument("--test-dir", type=Path, default=None, help="Defaults to <data-root>/test")
    ...
    parser.add_argument("--output", type=Path, default=BASE_DIR / "result.csv")
    return parser
...
def stack_features(paths: List[Path], cache_dir: Path):
    hist_vectors = [load_hist_features(path, cache_dir=cache_dir, split="test") for path in paths]
    fft_vectors = [load_fft_features(path, cache_dir=cache_dir, split="test") for path in paths]
...
def main() -> None:
    ...
    with torch.no_grad():
        for images, names, paths in loader:
            hist_feats, fft_feats = stack_features(list(paths), args.cache_dir)
            ...
            if moe_model:
                ...
                logits = moe_model(expert_logits, confidences=confidences)
            else:
                logits = (hist_logits + fft_logits + cnn_logits) / 3.0
            preds = torch.argmax(logits, dim=1).cpu().numpy()
            for name, pred in zip(names, preds):
                predictions.append((name, IDX_TO_SUBMISSION[int(pred)]))

    args.output.parent.mkdir(parents=True, exist_ok=True)
    with args.output.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["ImageID", "Label"])
        writer.writerows(sorted(predictions, key=lambda x: x[0]))
