...
from features import FFT_FEATURE_DIM
from models import FFTMLP
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train FFT expert (Expert B).")
...
def main() -> None:
    ...
    train_dataset = FeatureDataset(args.data_root / "train", "fft", cache_dir=args.cache_dir, split="train")
    val_dataset = FeatureDataset(args.data_root / "val", "fft", cache_dir=args.cache_dir, split="val")
    ...
    model = FFTMLP(FFT_FEATURE_DIM).to(device)
    ...
    if wandb_run:
        ...
        log_images_to_wandb(wandb_run, "fft_expert_samples", random_samples, CLASSES)

    best_acc = 0.0
    artifact_path = args.artifacts_dir / "fft_expert.pth"
